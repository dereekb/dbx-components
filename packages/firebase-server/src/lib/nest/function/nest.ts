...
// MARK: Event
export type FirestoreEventHandler<I = any, O = any> = (data: I, context: EventContext) => Promise<O>;
export type NestApplicationEventHandler<I = any, O = any> = (nest: INestApplicationContext, data: I, context: EventContext) => Promise<O>;
 ...
 */
export type OnEventWithNestContextFactory<C> = <I, O>(fn: OnEventWithNestContextBuilder<C, I, O>) => NestApplicationFunctionFactory<functions.CloudFunction<O>>;
...
export function onEventWithNestContextFactory<C>(makeNestContext: MakeNestContext<C>): OnEventWithNestContextFactory<C> {
  return <I, O>(fn: OnEventWithNestContextBuilder<C, I, O>) => {
    return (nestAppPromiseGetter: NestApplicationPromiseGetter) => {
