 ...
 * @returns 
 */
export function dataFromSnapshotStream<T>(stream: Observable<DocumentSnapshot<T>>): Observable<T> {
  return stream.pipe(mapDataFromSnapshot(), filterMaybe());
}
 ...
 * @returns 
 */
export function mapDataFromSnapshot<T>(): OperatorFunction<DocumentSnapshot<T>, Maybe<T>> {
  return map((x) => x.data());
}
