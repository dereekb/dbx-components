...
export async function runCliFirestoreQuery(input: RunCliFirestoreQueryInput): Promise<CliFirestoreQueryResult> {
  ...
  // refused BEFORE the constraints are built and before Firestore is touched: the mode is a
  // property of the entry, so paying a round trip to be told `permission-denied` teaches nothing.
  // `parent` matters here — a parent-child entry is fine once scoped to one parent document.
  ...
  const collection = cliFirestoreCollectionForQuery({
    ...
    ...(entry.rules?.parentPaths === undefined ? {} : { parentPaths: entry.rules.parentPaths })
  });
