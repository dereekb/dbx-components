...
export function createCli(input: CreateCliInput): Argv {
  ...
  if (input.version == null) {
    ...
    parser = parser.version(cliVersionOutput({ version: input.version, rawArgv, ...(input.manifestGeneratorVersion == null ? {} : { manifestGeneratorVersion: input.manifestGeneratorVersion }) }));
  }
...
interface CliVersionOutputInput {
  readonly version: string;
 ...
 */
function cliVersionOutput(input: CliVersionOutputInput): string {
  const { version, rawArgv, manifestGeneratorVersion } = input;
  const wantsVersion = rawArgv.includes('--version') || rawArgv.includes('-V');
  let result = version;
