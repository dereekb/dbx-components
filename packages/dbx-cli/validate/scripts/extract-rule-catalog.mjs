 ...
 * packages/dbx-cli/validate/src/generated/rule-catalog.generated.json
 * (reference copy) plus the typed re-export
 * packages/dbx-cli/validate/src/generated/rule-catalog.generated.ts
 * (the `RULE_CATALOG` const consumed at runtime).
 *
 * Walks every `src/lib/<cluster>/codes.ts` (including the special-cased
 * `model-fixture-shared/codes.ts`) using ts-morph, reads the JSDoc
 * summary + `@dbxRule*` tags off each enum member, and assembles the
 * `RuleEntry[]` shape declared in `src/lib/_core/rule-catalog/types.ts`.
 *
...
const TOOLS_ROOT = join(WORKSPACE_ROOT, 'packages/dbx-cli/validate/src/lib');
const OUTPUT_JSON = join(WORKSPACE_ROOT, 'packages/dbx-cli/validate/src/generated/rule-catalog.generated.json');
const OUTPUT_TS = join(WORKSPACE_ROOT, 'packages/dbx-cli/validate/src/generated/rule-catalog.generated.ts');
...
const CLUSTER_TO_SOURCE = {
  ...
  'model-firebase-index-validate-app': 'dbx_model_firebase_index_validate_app',
  'model-server-only-validate-app': 'dbx_model_server_only_validate_app',
  'model-fixture-shared': 'dbx_model_fixture_validate_app',
  'model-test-validate-app': 'dbx_model_test_validate_app',
...
async function main() {
  ...
  const tsFormatted = await formatWithOxfmt(tsRaw);
...
function emitTs(entries) {
  ...
  return ['// THIS FILE IS GENERATED by scripts/extract-rule-catalog.mjs.', '// Do not edit by hand. Run `npx nx generate-rule-catalog dbx-cli-validate`.', '', "import type { RuleEntry } from '../lib/_core/rule-catalog/types.js';", '', `export const RULE_CATALOG: readonly RuleEntry[] = ${json};`, ''].join('\n');
}

/**
 * Runs the workspace's oxfmt config against the generated source so the output
 * matches what `oxfmt --write` would produce on the committed file. oxfmt's
 * programmatic `format` does not discover config files the way the CLI does,
 * so the workspace `.oxfmtrc.json` is loaded explicitly.
 */
async function formatWithOxfmt(source) {
  ...
  const { code, errors } = await format(OUTPUT_TS, source, config);

  if (errors.length > 0) {
    throw new Error(`oxfmt failed to format ${relPath(OUTPUT_TS)}: ${errors.map((x) => x.message).join(', ')}`);
  }
