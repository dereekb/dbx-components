...
function renderModelEntry(entry: CliModelManifestEntry, emitConverters: boolean): string {
  const fields: (string | undefined)[] = [
    ...
    entry.singleton ? 'singleton: true' : undefined,
    entry.singleItemIdentifier === undefined ? undefined : `singleItemIdentifier: ${JSON.stringify(entry.singleItemIdentifier)}`,
    entry.exampleKey === undefined ? undefined : `exampleKey: ${JSON.stringify(entry.exampleKey)}`,
    entry.description ? `description: ${JSON.stringify(entry.description)}` : undefined,
    ...
    entry.compositeKey ? `compositeKey: { from: ${JSON.stringify(entry.compositeKey.from)}, encoding: ${JSON.stringify(entry.compositeKey.encoding)} }` : undefined
  ];
  return `  { ${fields.filter(Boolean).join(', ')} }`;
}
