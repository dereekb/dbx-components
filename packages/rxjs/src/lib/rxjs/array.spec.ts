...
describe('distinctUntilArrayLengthChanges()', () => {
  it(
    'should not emit values while the length does not change.',
    callbackTest((done) => {
      const value: [] = [];
  ...
  it(
    'should emit values if the same array is pushed and the length changes.',
    callbackTest((done) => {
      const value: number[] = [];
  ...
  it(
    'should use the mapping function to return the target array.',
    callbackTest((done) => {
      const value: {
        x: number[];
      } = {
