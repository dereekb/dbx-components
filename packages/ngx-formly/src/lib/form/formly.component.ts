...
import { FormComponentEvent, FormComponentState } from './form.component';
...
export class DbNgxFormlyComponent<T extends object> extends AbstractSubscriptionDirective implements DbNgxFormlyDirectiveDelegate<T>, OnInit, OnDestroy {
  ...
  private _events = new BehaviorSubject<FormComponentEvent>({ isComplete: false, state: FormComponentState.INITIALIZING });
  ...
  private _updateForChange(): void {
    ...
    const nextState: FormComponentEvent = {
      isComplete: complete,
