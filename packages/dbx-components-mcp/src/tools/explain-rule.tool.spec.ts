...
describe('dbx_explain_rule', () => {
  it('lists the full catalog when code is omitted', async () => {
    const result = (await EXPLAIN_RULE_TOOL.run({})) as { isError?: boolean; content: { text: string }[] };
  ...
  it('filters list mode by source', async () => {
    const result = (await EXPLAIN_RULE_TOOL.run({ source: 'dbx_model_validate_folder' })) as { isError?: boolean; content: { text: string }[] };
    expect(result.isError).toBeFalsy();
    const text = result.content[0].text;
  ...
  it('filters list mode by severity', async () => {
    const result = (await EXPLAIN_RULE_TOOL.run({ severity: 'warning' })) as { isError?: boolean; content: { text: string }[] };
    expect(result.isError).toBeFalsy();
    const text = result.content[0].text;
  ...
  it('returns JSON in list mode when format=json', async () => {
    const result = (await EXPLAIN_RULE_TOOL.run({ source: 'dbx_model_validate_folder', format: 'json' })) as { isError?: boolean; content: { text: string }[] };
    expect(result.isError).toBeFalsy();
  ...
  it('returns isError when filters match no rules', async () => {
    const result = (await EXPLAIN_RULE_TOOL.run({ source: 'dbx_bogus_source' })) as { isError?: boolean; content: { text: string }[] };
    expect(result.isError).toBe(true);
    expect(result.content[0].text).toContain('No rules match');
  });

  it('renders a known code as markdown with all sections', async () => {
    const result = (await EXPLAIN_RULE_TOOL.run({ code: 'FOLDER_NOT_FOUND' })) as { content: { text: string }[]; isError?: boolean };
    expect(result.isError).toBeFalsy();
    const text = result.content[0].text;
  ...
  it('returns the code in JSON when format=json', async () => {
    const result = (await EXPLAIN_RULE_TOOL.run({ code: 'FOLDER_NOT_FOUND', format: 'json' })) as { content: { text: string }[]; isError?: boolean };
    expect(result.isError).toBeFalsy();
    const parsed = JSON.parse(result.content[0].text);
  ...
  it('falls back to candidate suggestions on an unknown code', async () => {
    const result = (await EXPLAIN_RULE_TOOL.run({ code: 'FOLDER_BOGUS' })) as { content: { text: string }[]; isError?: boolean };
    expect(result.isError).toBe(true);
    const text = result.content[0].text;
