 ...
 * to callers. Violations are grouped by side (component / api / both) so
 * readers see which package needs the fix.
 *
...
export function formatResult(result: ValidationResult): string {
  ...
  lines.push('');
  lines.push("_Run `dbx_artifact_file_convention` with the relevant artifact (e.g. `{artifact: 'storagefile-purpose'}`, `'storagefile-upload-handler'`, `'storagefile-processor'`, `'storagefile-processor-subtask'`) to see canonical paths and wiring for missing pieces._");
