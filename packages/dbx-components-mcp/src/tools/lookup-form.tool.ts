...
import { FORM_TIER_ORDER, type FormFieldInfo, type FormTier } from '../registry/index.js';
...
const DBX_FORM_LOOKUP_TOOL: Tool = {
  ...
  description: [
    ...
    "  • a form tier name (`'field-factory'`, `'field-derivative'`, `'composite-builder'`, `'template-builder'`, `'primitive'`) to list every entry in that tier;",
 ...
 */
export interface CreateLookupFormToolConfig {
...
export function createLookupFormTool(config: CreateLookupFormToolConfig): DbxTool {
  ...
  function resolveTierTopic(lowered: string): LookupFormMatch | undefined {
    return FORM_TIER_ORDER.includes(lowered as FormTier) ? { kind: 'group', title: `Form entries: tier = ${lowered}`, fields: registry.findByTier(lowered as FormTier) } : undefined;
  }

  function resolveDirectFieldTopic(rawTopic: string, lowered: string): LookupFormMatch | undefined {
    const directHit = findField(rawTopic) ?? findField(lowered);
  ...
  function resolveAliasFieldTopic(aliased: string, lowered: string): LookupFormMatch | undefined {
    const aliasHit = aliased === lowered ? undefined : findField(aliased);
  ...
  function resolveProducesTopic(rawTopic: string): LookupFormMatch | undefined {
    ...
    return produces ? { kind: 'group', title: `Form entries producing \`${produces}\``, fields: registry.findByProduces(produces) } : undefined;
  }
  ...
  function findProducesMatch(topic: string): string | undefined {
    ...
    return registry.producesCatalog.find((v) => v.toLowerCase() === lowered);
  }
  ...
  function formatCatalog(): string {
    const lines: string[] = ['# Form catalog', '', `${registry.all.length} entries across ${FORM_TIER_ORDER.length} tiers.`, ''];
    for (const tier of FORM_TIER_ORDER) {
      const list = registry.findByTier(tier);
    ...
    for (const value of registry.producesCatalog) {
      const count = registry.findByProduces(value).length;
