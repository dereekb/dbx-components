/**
 * Validation rules run against a {@link NotificationFolderInspection}.
 * Rules accumulate {@link Violation}s into a mutable buffer; the
 * public entry point is {@link validateNotificationFolder} in
 * `./index.ts`.
 */
...
function checkSidePresence(side: SideInspection, violations: Violation[], dirCode: 'NOTIF_FOLDER_COMPONENT_DIR_NOT_FOUND' | 'NOTIF_FOLDER_API_DIR_NOT_FOUND', folderCode: 'NOTIF_FOLDER_COMPONENT_FOLDER_MISSING' | 'NOTIF_FOLDER_API_FOLDER_MISSING', subPath: string): void {
  if (side.status === 'dir-not-found') {
    ...
    pushViolation(violations, {
      code: folderCode,
      message: `Notification folder \`${subPath}\` is missing under \`${side.rootDir}\`.`,
      side: side.side,
      file: undefined
    });
  }
...
function checkUnexpectedFileNames(side: SideInspection, violations: Violation[]): void {
  for (const file of side.files) {
    ...
    if (file.startsWith(NOTIFICATION_FILE_PREFIX)) continue;
    pushViolation(violations, {
      code: 'NOTIF_FOLDER_UNEXPECTED_FILE_NAME',
      severity: 'warning',
      message: `File \`${file}\` does not start with \`${NOTIFICATION_FILE_PREFIX}\`. Notification-folder files should be named \`${NOTIFICATION_FILE_PREFIX}<sub>.ts\` to stay grouped.`,
      side: side.side,
      file
    });
  }
...
function checkHandlersSubfolderMix(side: SideInspection, violations: Violation[]): void {
  ...
  for (const file of side.files) {
    ...
    pushViolation(violations, {
      code: 'NOTIF_FOLDER_HANDLERS_SUBFOLDER_MIXED',
      severity: 'warning',
      message: `File \`${file}\` lives at the notification folder root alongside a \`${HANDLERS_SUBFOLDER_NAME}/\` subfolder. Move handler files into \`${HANDLERS_SUBFOLDER_NAME}/\` to keep the convention consistent.`,
      side: side.side,
      file
    });
  }
  ...
  while (match !== null) {
    ...
    if (!seen.has(normalized)) {
      ...
      if (!fileSet.has(fileMatch) && !entrySet.has(normalized)) {
        pushViolation(violations, {
          code: 'NOTIF_FOLDER_BARREL_REEXPORT_MISSING',
