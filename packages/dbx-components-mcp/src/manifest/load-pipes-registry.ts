 ...
 */
export async function loadPipeRegistry(input: LoadPipeRegistryInput): Promise<LoadPipeRegistryResult> {
  const { cwd, bundledManifestPaths = DEFAULT_BUNDLED_PATHS, readFile } = input;

  const bundledSources: PipeManifestSource[] = bundledManifestPaths().map((path) => ({ origin: 'bundled', path }));

  const configResult = await findAndLoadConfig({ cwd, readFile });
  const externalSources: PipeManifestSource[] = [];
  if (configResult.config !== null && configResult.configPath !== null) {
    const baseDir = dirname(configResult.configPath);
    const declared = configResult.config.pipes?.sources ?? [];
  ...
  let registry: PipeRegistry = EMPTY_PIPE_REGISTRY;
  let loaderWarnings: readonly PipeLoaderWarning[] = [];

  const sources: readonly PipeManifestSource[] = [...bundledSources, ...externalSources];
  if (sources.length > 0) {
    const loaded = await loadPipeManifests({ sources, readFile });
    registry = createPipeRegistry(loaded);
    loaderWarnings = loaded.warnings;
  }

  const result: LoadPipeRegistryResult = {
    registry,
 ...
 */
export function getDefaultBundledPipeManifestPaths(): readonly string[] {
