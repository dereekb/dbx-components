...
export async function loadForgeFieldRegistry(input: LoadForgeFieldRegistryInput): Promise<LoadForgeFieldRegistryResult> {
  ...
  const bundledSources: ForgeFieldManifestSource[] = bundledManifestPaths().map((path) => ({ origin: 'bundled', path }));

  const configResult = await findAndLoadConfig({ cwd, readFile });
  const externalSources: ForgeFieldManifestSource[] = [];
  if (configResult.config !== null && configResult.configPath !== null) {
    const baseDir = dirname(configResult.configPath);
    const declared = configResult.config.forgeFields?.sources ?? [];
  ...
  let registry: ForgeFieldRegistry = EMPTY_FORGE_FIELD_REGISTRY;
  let loaderWarnings: readonly ForgeFieldLoaderWarning[] = [];

  const sources: readonly ForgeFieldManifestSource[] = [...bundledSources, ...externalSources];
  if (sources.length > 0) {
    const loaded = await loadForgeFieldManifests({ sources, readFile });
    registry = createForgeFieldRegistry(loaded);
    loaderWarnings = loaded.warnings;
  }

  const result: LoadForgeFieldRegistryResult = {
    registry,
