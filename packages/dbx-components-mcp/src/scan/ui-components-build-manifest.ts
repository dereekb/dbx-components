 ...
 */
export type BuildUiManifestReadFile = (absolutePath: string) => Promise<string>;
 ...
 */
export type BuildUiManifestGlobber = (input: { readonly projectRoot: string; readonly include: readonly string[]; readonly exclude: readonly string[] }) => Promise<readonly string[]>;
...
const DEFAULT_READ_FILE: BuildUiManifestReadFile = (path) => nodeReadFile(path, 'utf-8');

const DEFAULT_GLOBBER: BuildUiManifestGlobber = async (input) => {
...
type LoadPackageNameResult = { readonly kind: 'ok'; readonly packageName: string } | { readonly kind: 'fail'; readonly outcome: Extract<BuildUiManifestOutcome, { kind: 'no-package' | 'invalid-package' }> };

async function loadPackageName(packagePath: string, readFile: BuildUiManifestReadFile): Promise<LoadPackageNameResult> {
  let raw: string | null = null;
  try {
 ...
 * semantic-types builder so both pipelines apply exclude rules
 * identically.
 *
