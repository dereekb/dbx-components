...
import { forgeWebsiteUrlField } from './website';

// MARK: forgeWebsiteUrlField
describe('forgeWebsiteUrlField()', () => {
  ...
  it('should allow overriding the key', () => {
    const field = forgeWebsiteUrlField({ key: 'homepage' });
    expect(field.key).toBe('homepage');
  });

  it('should allow overriding the label', () => {
    const field = forgeWebsiteUrlField({ label: 'Homepage' });
    expect(field.label).toBe('Homepage');
  });

  it('should set required when specified', () => {
    const field = forgeWebsiteUrlField({ required: true });
    expect(field.required).toBe(true);
  });

  it('should map description to hint in props', () => {
    const field = forgeWebsiteUrlField({ description: 'Enter your website' });
