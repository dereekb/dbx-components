...
import { forgeDragArrayField, FORGE_DRAG_ARRAY_FIELD_TYPE_NAME } from './dragarray.field';

describe('forgeDragArrayField()', () => {
  it('should create a field with the correct type', () => {
    ...
    expect(field.type).toBe(FORGE_DRAG_ARRAY_FIELD_TYPE_NAME);
  });
  ...
  it('should pass template through props', () => {
    ...
    const field = forgeDragArrayField({ key: 'items', template });
  ...
  it('should pass single-field template', () => {
    ...
    const field = forgeDragArrayField({ key: 'tags', template });
    expect(field.props?.template).toEqual(template);
  });
