import { Directive, Host, OnInit, OnDestroy, Input, ViewContainerRef } from '@angular/core';
...
import { delay, first, map, mergeMap, startWith, tap } from 'rxjs/operators';
import { ActionContextStoreSourceInstance } from './action';
import { ActionContextStore, ActionState, canTriggerAction, canTriggerActionState, isIdleActionState } from './action.store';
import { DbNgxActionTransitionSafetyDialogComponent, DbNgxActionTransitionSafetyDialogResult } from './transition.safety.dialog.component';
...
export class DbNgxActionTransitionSafetyDirective<T, O> implements OnInit, OnDestroy {
  ...
  private _dialogRef: MatDialogRef<DbNgxActionTransitionSafetyDialogComponent, DbNgxActionTransitionSafetyDialogResult>;
  ...
  private _showDialog(transition: Transition): Observable<HookResult> {
    ...
    if (!this._dialogRef) {
      this._dialogRef = this.dialog.open(DbNgxActionTransitionSafetyDialogComponent, {
