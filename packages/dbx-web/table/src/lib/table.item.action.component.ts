...
export class DbxTableItemActionComponent<T> extends AbstractDbxTableElementComponent<T> {
  readonly config$ = this.tableStore.viewDelegate$.pipe(
    switchMap((viewDelegate) => this.element$.pipe(map((x) => viewDelegate.itemAction(x)))),
