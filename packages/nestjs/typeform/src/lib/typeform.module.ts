...
import { TYPEFORM_TOKEN_ENV_VAR, TYPEFORM_BASE_URL_ENV_VAR, TypeformServiceConfig } from './typeform.config';
...
export function typeFormServiceConfigFactory(configService: ConfigService): TypeformServiceConfig {
  const config: TypeformServiceConfig = {
    typeform: {
      config: {
        token: configService.get<TypeformApiToken>(TYPEFORM_TOKEN_ENV_VAR) as TypeformApiToken,
