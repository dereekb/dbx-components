import { type NotificationTaskService, type NotificationTaskServiceTaskHandlerConfig, notificationTaskService, storageFileProcessingNotificationTaskHandler, type StorageFileProcessingPurposeSubtaskProcessorConfig } from '@dereekb/firebase-server/model';
...
export function APP_CODE_PREFIX_CAMELNotificationTaskServiceFactory(APP_CODE_PREFIX_CAMELFirebaseServerActionsContext: APP_CODE_PREFIXFirebaseServerActionsContext): NotificationTaskService {
  ...
  const handlers: NotificationTaskServiceTaskHandlerConfig<any>[] = [storageFileHandler];

  const notificationSendService: NotificationTaskService = notificationTaskService({
    validate: [...ALL_STORAGE_FILE_NOTIFICATION_TASK_TYPES],
    handlers
  });
