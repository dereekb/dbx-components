...
export const updateProfile: APP_CODE_PREFIXUpdateModelFunction<UpdateProfileParams> = withApiDetails({
  fn: async (request) => {
...
export const updateProfleOnboarding: APP_CODE_PREFIXUpdateModelFunction<FinishOnboardingProfileParams, boolean> = withApiDetails({
  fn: async (request) => {
    const { nest, auth, data } = request;
    const uid = auth.uid;
