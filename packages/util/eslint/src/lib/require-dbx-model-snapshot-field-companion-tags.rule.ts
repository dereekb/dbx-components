...
export interface UtilRequireDbxModelSnapshotFieldCompanionTagsRuleDefinition {
  ...
  create(context: { options: UtilRequireDbxModelSnapshotFieldCompanionTagsRuleOptions[]; report: (descriptor: { loc?: AstNode; messageId: string; data?: Record<string, string>; fix?: (fixer: AstNode) => Maybe<AstNode | AstNode[]> }) => void; sourceCode: AstNode }): Record<string, (node: AstNode) => void>;
}
