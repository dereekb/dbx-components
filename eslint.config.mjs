...
export default [
  ...
  },
  {
    ...
    rules: {
      ...
      // FORM_SPACE_PURPOSE is opted out of PATH folding (only): its destination is keyed by the FormSpace
      // id and slot supplied at request time, so there is no constant path to fold. The size/mime mirror
      // against `storage.rules` still applies.
      'dereekb-firebase/require-storagefile-policy-matches-rules': ['warn', { allowUnresolvablePolicies: ['FORM_SPACE_PURPOSE'] }], // cross-check STORAGE_FILE_PURPOSE_UPLOAD_POLICIES entries against the workspace `storage.rules` — each policy's `maxFileSizeBytes` and `allowedMimeTypes` must match the paired `// Mirrors STORAGE_FILE_PURPOSE_UPLOAD_POLICIES[<KEY>]` rule block so signed-upload URLs never sign a request the bucket then rejects
