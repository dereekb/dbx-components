...
import { APP_CODE_PREFIXFirebaseBaseContext, APP_CODE_PREFIX_LOWERFirebaseModelServices, APP_CODE_PREFIXFirestoreCollections, ProfileDocument, ProfileRoles } from 'FIREBASE_COMPONENTS_NAME';

/**
 * Used to access the APP_CODE_PREFIX_LOWERFirebaseModelServices() on the client side.
 */
@Injectable()
export class APP_CODE_PREFIXFirebaseContextService implements DbxFirebaseModelContextService {

  readonly APP_CODE_PREFIX_LOWERFirestoreCollections = inject(APP_CODE_PREFIXFirestoreCollections);
  ...
  readonly baseContext$: Observable<APP_CODE_PREFIXFirebaseBaseContext> = this.dbxFirebaseAuthService.currentAuthContextInfo$.pipe(
    map((auth) => {
      const result: APP_CODE_PREFIXFirebaseBaseContext = {
        auth,
        app: this.APP_CODE_PREFIX_LOWERFirestoreCollections
      };
  ...
  readonly context$ = this.baseContext$.pipe(map((x) => inContextFirebaseModelsServiceFactory(APP_CODE_PREFIX_LOWERFirebaseModelServices)(x)));
