...
- `dbx-claude-lint-run` / `dbx-claude-lint-read` drive **either** tier via `linter: 'eslint' | 'oxlint'` (default `eslint`, so an unqualified call is unchanged). Each tier reads/writes its own cache, so one never clobbers the other.
- **`dbx-claude-commit` blocks on BOTH tiers.** `.dbx-claude/dbx-claude.json` sets `commit.linters: ["eslint", "oxlint"]`, and an error-severity finding from either engine that lands on a changed file fails the gate identically. The workspace is at 0 oxlint errors, so this starts green and costs ~2.5 s.
