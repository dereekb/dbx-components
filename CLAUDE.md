...
- Whole workspace: `npx nx run workspace:oxlint-all` (~2.5 s) and `npx nx run workspace:lint-all` (~30 s warm, ~110 s cold). The run immediately after **any** edit to a root ESLint config costs ~1 m 50 s instead of ~25 s (Nx re-runs plugin inference) — discard one settle run before timing anything, or a no-op change reads as a 4x regression.
...
- **The five in-repo `dereekb-*` plugins stay on ESLint entirely — porting them is not pending, it was measured and rejected. Measured 2026-09-03:** 34 of their 56 rule names DO run unmodified under `jsPlugins` (byte-identical message and `line:col`), but moving them changes `lint-all` by **less than the ±1.5 s run-to-run noise** — all 45 active first-party rules cost **101.6 ms of a 2.65 s rule budget** on `packages/date`, while ~58 % of a lint invocation is startup + TypeScript program construction and five type-aware/import-graph rules are 75 % of rule time. `jsPlugins` is also still alpha at oxlint 1.81.0, which `npm view oxlint dist-tags` confirms is the **latest published** version. Do not re-open without re-measuring — see `~/.claude/plans/port-the-dereekb-eslint-rules-to-oxlint-where-compatible.md` and §3.1b of `adopt-the-nx-oxlint-plugin.md`.
