...
A dynamically registered client requests the protected-resource document's `scopes_supported` verbatim, and its base list — `OidcProviderConfigService.clientRequestableScopesSupported` — drops the scopes an admin unlocks per-client by assigning an [OIDC provider profile](packages/firebase/src/lib/common/auth/oidc/oidc.profile.ts) (`lms`, `reports`, `token.cli` in the demo). A resource may still advertise one deliberately, as demo-api does for `token.cli`: the consent URL builder withholds an assignment-only scope from any client whose profiles do not unlock it, so the scope lands in the consent submit's `rejected` set and the flow completes without it, exactly as an admin-only scope does for a non-admin. The unlock gate then judges what was *consented to* rather than what was requested, and stays as defense in depth.

So advertising a gated scope is safe for every client, and the switch that decides who can actually obtain it is the profile assignment (`dbx_provider_profiles`), not the document. A connector that should mint CLI credentials needs the `cli-handoff` profile assigned to its client and an admin user — advertising `token.cli` alone grants nothing.
